      ...
      >
        ...
        <div :key="item.updateKey" style="height: 100%">
            ...
            <Echarts
              ...
              :hasDataLabels="item.hasDataLabels"
              ...
              :subtitle="getItemPeriodText(item)"
              :title="item.name"
              :yAxisText="item.hasLabels ? item.yAxis : ''"
              :xAxisText="item.hasLabels ? item.xAxis : ''"
              :height="item.h * 50"
              @delete-diagram="showDeleteWarning(item.id)"
              @edit-diagram="editDiagram(item.diagramId)"
              @copy-diagram="openDiagramCopyDialog(item.diagramId)"
            ...
            <!-- <Echarts
              ...
              @copy-diagram="openDiagramCopyDialog(item.diagramId)"
              v-if="
                item.graphType !== 'trend' &&
                item.graphType !== 'grid' &&
                item.graphType !== 'area' &&
                item.graphType !== 'line_timeline' &&
                item.graphType !== 'net' &&
                item.graphType !== 'sankey' &&
                item.graphType !== 'counter' &&
                item.graphType !== 'pulse' &&
                item.graphType !== 'stacked area' &&
                $store.state.periodsInt.find((el) => item.periodId === el.id)
              "
            /> -->
  ...
  export default {
    ...
    methods: {
      getItemPeriodText(item) {
        return `с ${item.dateFrom} до ${item.dateTo}`
        // if (item.dateFrom) {
